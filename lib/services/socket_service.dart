...
///
/// Ye service backend ke Socket.io server se connect hoti hai
/// aur real-time events receive/send karta hai jaise booking updates
class SocketService {
  // Singleton pattern - Sirf ek instance rahega poore app mein
  ...
  String? _currentUserId; // Persistent User ID for persistent room subscription
  String? _pendingRoomJoin; // Pending room join ID
  Completer<void>? _connectionCompleter; // Connection complete hone ka wait

  // Stream controllers for events - UI ko update karne ke liye
  ...
  // Chat-specific stream controllers
  ...
  // Public streams - UI mein inhe listen karein
  ...
  ///
  /// Backend server (port 5000) ke Socket.io endpoint se connect karta hai
  /// Auth token bhejta hai taake server user identify kar sake
  Future<void> connect() async {
    // Vercel serverless platform does not host persistent WebSocket servers.
    // Disable WebSocket socket.io connection on Web Chrome when pointing to Vercel production.
    ...
    // Pehle purana connection clean karo
    if (_socket != null) {
      _socket!.disconnect();
      _socket = null;
    }
    ...
    try {
      // Auth token lo - backend authentication ke liye
      final token = await TokenManager.getToken();

      debugPrint('🔑 Token: ${token != null ? 'Found' : 'Not found'}');
      debugPrint('🌐 Connecting to: ${ApiConstants.socketBaseUrl}');

      // Socket.io client initialize karo
      _socket = socket_io.io(
        ApiConstants.socketBaseUrl,
        socket_io.OptionBuilder()
            .setTransports(['websocket', 'polling'])
            .setReconnectionAttempts(3) // Reconnection attempts limit to avoid infinite loops on Web/Vercel
            .setAuth({'token': token})
            .build(),
      );

      // Connection event handlers setup karo
      _setupEventHandlers();

      // Manual connect call karo
      _socket!.connect();
      debugPrint('🔄 Connecting to socket server...');

      // 3 seconds tak wait karo connection ke liye (fast fallback to HTTP for Web/Serverless)
  ...
  /// Socket event handlers setup karo
  ///
  /// Ye functions backend se aane wale events ko handle karte hain
  void _setupEventHandlers() {
    if (_socket == null) return;

    // 1. Jab socket connect ho jaye
    _socket!.onConnect((_) {
      debugPrint('✅ Socket connected successfully');
      _isConnected = true;
      _connectionStatusController.add(true);
      
      // Connection complete signal - only complete if not already completed
      if (_connectionCompleter != null && !_connectionCompleter!.isCompleted) {
        _connectionCompleter!.complete();
      }

      // Auto re-join user room on every connect/reconnect
    ...
    // 2. Jab socket disconnect ho jaye
    ...
    // 3. Jab connection error aaye
    _socket!.onConnectError((error) {
      debugPrint('⚠️ Socket connect error: $error (HTTP fallback active)');
      _isConnected = false;
      _connectionStatusController.add(false);
      // Connection complete signal - complete gracefully for HTTP fallback
      if (_connectionCompleter != null && !_connectionCompleter!.isCompleted) {
        _connectionCompleter!.complete();
      }
    });

    // 4. Backend se 'booking_created' event receive karo
    //    Ye tab aata hai jab customer nayi booking create karta hai
    //    Provider ko notification milta hai
    ...
    // 5. Backend se 'booking_status_updated' event receive karo
    //    Ye tab aata hai jab booking status change hota hai
    //    (pending -> accepted -> completed etc.)
    //    Dono customer aur provider ko update milta hai
    ...
    // 6. Backend se persisted notification receive karo
  ...
  /// Robust Map parser helper for socket payloads
  ...
  /// User ki private room join karo
  ///
  /// Backend mein har user ki ek private room hoti hai (user ID ke naam se)
  /// Jab user login karta hai to apni room join karta hai
  /// Tab backend us user ko specific events bhej sakta hai
  ///
  /// Usage: SocketService().joinRoom(userId);
  void joinRoom(String userId) {
    ...
    // Backend ko bhejo ke ye user apni room join kar raha hai
  ...
  /// Generic on method for custom event listeners
  ...
  /// Socket connection disconnect karo
  ///
  /// Jab user logout kare ya app band ho, to connection close karo
  void disconnect() {
    if (_socket != null) {
      _socket!.disconnect();
      _socket = null;
  ...
  /// Cleanup resources
  ///
  /// App close hone pe streams dispose karo
  void dispose() {
